...
def bin_to_hex(bin_str):
    ...
    for i in range(0, len(bin_str), 8):
        b = bin_str[i : i + 8]
        h = hex(int(b, 2))[2:].zfill(2)
        hex_str += h
    return hex_str


def hex_str_to_int_array(hex_str):
    # 去除字符串两端的空白符，并以逗号为分隔符分割字符串
    hex_nums = hex_str.strip().split(",")
    # 初始化一个空列表来存储转换后的整数
    int_array = []
    # 遍历每个16进制数
    for hex_num in hex_nums:
        # 去除"0x"前缀，并将字符串从16进制转换为整数
        int_num = int(hex_num.replace("0x", ""), 16)
        # 将转换后的整数添加到列表中
...
if __name__ == "__main__":
    ...
    # 2进 a           b  c  d e    f  g h i  j  k l m
    b = b"01001001010 00 10 0 0011 00 1 1 00 00 0 1 00"

    s = """0xFF,0xF2,0x40,0xC0,0x19,0xB7,0x00,0x14,0x02,0xE6,0x5C,
    ...
    0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,
    0x00"""
    # 将s按照,分割，并转换为16进制字符串输出
    i = hex_str_to_int_array(s)
    print(i)
    # i为一个文件每一个字节的数值数组，写回文件
    # 写入文件
    with open("d:/UV/vsProject/soundsender/test/test.bin", "wb") as f:
        # 遍历i的每一个元素，将其转化为字节，写入文件
        for j in i:
            f.write(int_to_byte(j))
